<?php
class Activity_UrlAccessTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testActivityModelUrlsAreAccessControlled()
    {
        $defaultModuleControllers =  APPLICATION_PATH . '/modules/activity/controllers';
        ...
        foreach ($iterator as $fileInfo) {
            ...
            if (strpos($fileName, "Controller")) {
                $className = "Activity_" . rtrim($fileName, ".php");
                require_once APPLICATION_PATH . '/modules/activity/controllers/' . $fileName;
                $methods = (get_class_methods($className));
                foreach ($methods as $method) {
                   if (strpos($method, "Action")) {
                        $controllerWithoutSuffix = strtolower(substr_replace($className, "", -10));
                        $controllerWithoutSuffix = strtolower(substr_replace($controllerWithoutSuffix, "", 0, 9));
                       # echo "\nController without suffix is $controllerWithoutSuffix";
                        ...
                        $urlFound =  'activity/' . $controllerWithoutSuffix . "/" . $actionWithoutSuffix[0];
