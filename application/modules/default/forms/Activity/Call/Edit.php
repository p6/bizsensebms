...
class Core_Form_Activity_Call_Edit extends Core_Form_Activity_Call_Create
{
    protected $_callModel;

    public function __construct($callModel)
    {
            $this->_callModel = $callModel;
        parent::__construct();
    }
    ...
    {
        parent::init();
        $formValues = $this->_callModel->fetch();
        ...
        if($formValues['to_type'] == 1) {
            $formValues['lead_id'] = $formValues['to_type_id'];
        } else {
            $formValues['contact_id'] = $formValues['to_type_id'];
        }
