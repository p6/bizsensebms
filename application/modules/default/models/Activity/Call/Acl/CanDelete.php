...
/**
 * Validate whether the call can be deleted
 */
class Core_Model_Activity_Call_Acl_CanDelete extends Core_Model_Activity_Call_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($callRecord)) {
			if($acl->isAllowed($user, 'delete all calls')) {
				return true;
			} elseif($acl->isAllowed($user, 'delete own branch calls')) {
				if ($callRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'delete own role calls')) {
			...
			} elseif ($acl->isAllowed($user,'delete own calls')) {
