...
/** 
 * Validate whether the call can be edited
 */
class Core_Model_Activity_Call_Acl_CanEdit extends Core_Model_Activity_Call_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($callRecord)) {
			if($acl->isAllowed($user, 'edit all calls')) {
				return true;
			} elseif($acl->isAllowed($user, 'edit own branch calls')) {
				if ($callRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'edit own role calls')) {
			...
			} elseif ($acl->isAllowed($user, 'edit own calls')) {
