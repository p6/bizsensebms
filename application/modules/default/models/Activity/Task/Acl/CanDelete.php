...
/**
 * Validate whether the task can be deleted
 */
class Core_Model_Activity_Task_Acl_CanDelete extends Core_Model_Activity_Task_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($taskRecord)) {
			if($acl->isAllowed($user, 'delete all tasks')) {
				return true;
			} elseif($acl->isAllowed($user, 'delete own branch tasks')) {
				if ($taskRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'delete own role tasks')) {
			...
			} elseif ($acl->isAllowed($user,'delete own tasks')) {
