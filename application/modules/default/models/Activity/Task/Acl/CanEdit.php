...
/** 
 * Validate whether the task can be edited
 */
class Core_Model_Activity_Task_Acl_CanEdit extends Core_Model_Activity_Task_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($taskRecord)) {
			if($acl->isAllowed($user, 'edit all tasks')) {
				return true;
			} elseif($acl->isAllowed($user, 'edit own branch tasks')) {
				if ($taskRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'edit own role tasks')) {
			...
			} elseif ($acl->isAllowed($user, 'edit own tasks')) {
