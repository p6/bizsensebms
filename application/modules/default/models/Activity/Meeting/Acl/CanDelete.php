...
/**
 * Validate whether the meeting can be deleted
 */
class Core_Model_Activity_Meeting_Acl_CanDelete extends Core_Model_Activity_Meeting_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($meetingRecord)) {
			if($acl->isAllowed($user, 'delete all meetings')) {
				return true;
			} elseif($acl->isAllowed($user, 'delete own branch meetings')) {
				if ($meetingRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'delete own role meetings')) {
			...
			} elseif ($acl->isAllowed($user,'delete own meetings')) {
