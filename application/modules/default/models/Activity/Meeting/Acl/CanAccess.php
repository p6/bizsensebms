...
/** 
 * Validate whether the meeting can be accessed
 */
class Core_Model_Activity_Meeting_Acl_CanAccess extends Core_Model_Activity_Meeting_Acl_Abstract
{
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($meetingRecord)) {
			if($acl->isAllowed($user, 'view all meetings')) {
				return true;
			} elseif($acl->isAllowed($user, 'view own branch meetings')) {
				if ($meetingRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user,'view own role meetings')) {
			...
			} elseif ($acl->isAllowed($user, 'view own meetings')) {
