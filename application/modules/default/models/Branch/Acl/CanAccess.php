...
class Core_Model_Branch_Acl_CanAccess extends Core_Model_Branch_Acl_Abstract
{
    public $db;
	public $brancharray;
    ...
    /*
     * @return Bool
     * Check if the current user has access to view details of branch
     */
    public function assert(Bare_Acl $acl)
    {
		$user = $acl->getSubject(); 
		$branchRecord = $this->_branchRecord;

		if(!empty($branchRecord)) {
			if($acl->isAllowed($user,'access branch pages')) {
				return true;
		    } else {
    	    	return false;
			}
