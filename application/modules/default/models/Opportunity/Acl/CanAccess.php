...
/** 
 * Validate whether the oppotunity can be accessed
 */
class Core_Model_Opportunity_Acl_CanAccess extends Core_Model_Opportunity_Acl_Abstract
{
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($opportunityRecord)) {
			if($acl->isAllowed($user, 'view all opportunities')) {
				return true;
			} elseif($acl->isAllowed($user, 'view own branch opportunities')) {
				if ($opportunityRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user,'view own role opportunities')) {
			...
			} elseif ($acl->isAllowed($user, 'view own opportunities')) {
