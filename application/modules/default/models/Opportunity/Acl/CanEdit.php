...
/** 
 * Validate whether the oppotunity can be edited
 */
class Core_Model_Opportunity_Acl_CanEdit extends Core_Model_Opportunity_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($opportunityRecord)) {
			if($acl->isAllowed($user, 'edit all opportunities')) {
				return true;
			} elseif($acl->isAllowed($user, 'edit own branch opportunities')) {
				if ($opportunityRecord->branch_id == $user->getBranchId()){
				return true;
				}
            } elseif ($acl->isAllowed($user, 'edit own role opportunities')) {
			...
			} elseif ($acl->isAllowed($user, 'edit own opportunities')) {
