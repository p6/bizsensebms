 ...
 * @version    $Id:$
 */
class Core_Model_Finance_Receipt_Index extends Core_Model_Index_Abstract
{
    /*
    ...
    public function getPaginator()
    {
       $db = Zend_Registry::get('db');
        ...
        if ($search['mode'] == 'DD_CHECK') {
            $mode = '1';
            $select->where('mode = ?',$mode);
        }  
        
        if ($search['type'] == 'RECEIPT TO SUNDRY CREDITORS') {
            $type = '1';
            $select->where('type = ?',$type);
        }   
        
        if ($search['type'] == 'RECEIPT TOWARDS INDIRECT INCOME') {
            $type = '2';
            $select->where('type = ?',$type);
        } 
