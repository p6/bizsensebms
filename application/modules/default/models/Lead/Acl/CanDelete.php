...
class Core_Model_Lead_Acl_CanDelete extends Core_Model_Lead_Acl_Abstract
{
    ...
    public function assert(Bare_Acl $acl)
    {
		...
		if(!empty($leadRecord)) {
			if($acl->isAllowed($user, 'delete all leads')) {
				return true;
			} elseif($acl->isAllowed($user, 'delete own branch leads')) {
				if ($leadRecord->branch_id == $user->getBranchId()){
				    return true;
				}
			} elseif ($acl->isAllowed($user, 'delete own role leads')) {
            ...
            } elseif ($acl->isAllowed($user, 'delete own leads')) {
