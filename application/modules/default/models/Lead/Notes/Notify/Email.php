 ...
 * @version    $Id:$
 */
class Core_Model_Lead_Notes_Notify_Email 
     implements Core_Model_Observer_Interface
{
    ...
    public function getTextTable()
    {
        ...
        $createdDate = 
                 $viewHelper->timestampToHuman($this->_notesData['created']);        
        $textTable = new Zend_Text_Table(array('columnWidths' => array(20, 55)));
        $textTable->appendRow(array('Created on', $createdDate));
    ...
    public function getMessage($recepientName, $createdByUserFullName)
    {
        ...
        $url .= '/lead/notes/lead_id/' . $this->_notesData['lead_id'];
        $message = 'Hello ' . $recepientName . ', ' . "\n" . "\n";
        $message .=  $createdByUserFullName 
        . ' added notes to a lead. ' .
    ...
    {
        if ($observable->getStatus() != Core_Model_Lead_Notes::STATUS_CREATED) {
            return;
        }
        $this->_notesData = $observable->fetch();
        
        $subject = 'New Lead Note';
        ...
        $lead = new Core_Model_Lead($this->_notesData['lead_id']);
        $leadInfo = $lead->fetch();
        $assignedToUserId = $leadInfo->assigned_to;
